    ...
    void Update()
    {
        if (tooltip.enabled)
        {
            if (resource)
            {
                ...
                double micro = Math.Round(resource.microTimer - (resource.microTimer - resource.microPhase), 1);
                double macro = Math.Round(resource.macroTimer - (resource.macroTimer - resource.macroPhase), 1);
